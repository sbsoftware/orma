...
abstract class Orma::DbAdapters::Base
  ...
  abstract def unique_index_data_violation?(error : Exception) : Bool
  ...
  def create_unique_index_sql(table_name : String, index_name : String, columns : Array(String)) : String
    String.build do |io|
      io << "CREATE UNIQUE INDEX "
      io << index_name
      io << " ON "
      io << table_name
      io << " ("
